...
// Time complexity = O(n); function has to look at each word in the book
// Space complexity = O(n); memory will grow if algorithm is run multiple times
...
let wordFrequencies = (function () {
  ...
  return function (book, search) {
    ...
    for (var i = 0; i < book.length + 1; i++) {
      let char = book[i];
      if (char === ' ' || char === undefined || punctuation.indexOf(char) > -1) {
...
console.log(wordFrequencies(emancipation, 'virtue')) // => 2
console.log(wordFrequencies(emancipation, 'freedom')) // => 3
console.log(wordFrequencies(emancipation, 'assumption')) // => 1
console.log(wordFrequencies(emancipation, 'st')) // => 6 // St. Bernard, St. John, St. Charles, St. James, St. Mary, St. Martin
console.log(wordFrequencies(emancipation, 'st')) // => 6
console.log(wordFrequencies(emancipation, 'god')) // => 1
